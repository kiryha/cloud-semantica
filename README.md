...
### 2. The GenAI Pipeline: From Text to Latent Space
In modern AI architectures like **Diffusion Transformers (DiT)** (the tech behind Sora and Flux), the process works in three main stages:

*   **The Translator (Text Encoder):** This is where **Word Embeddings** live. The AI takes your written prompt and translates every word into a unique set of coordinates (a vector).
*   **The Playground (Latent Space):** Instead of trying to draw millions of tiny pixels at once, the AI works in a "compressed" version of reality called Latent Space. It’s a mathematical universe where concepts (not colors) are organized. 
*   **The Painter (Decoder):** Once the AI has "sculpted" the concept in Latent Space, it converts those coordinates back into actual pixels that our human eyes can see.

**Semantica (Language Math)** focuses on the first two steps: understanding how human language is mapped into that mathematical universe.



---

### 3. Word Embeddings: The Language of Coordinates
...
### 4. The 300-Dimensional "Point Cloud"
...
3.  **The Outlier:** Words like *apple*, *banana*, and *orange* exist in a tight "Fruit Cluster." *Meat*, however, is located in a distant part of the 300D space. Semantica flags the word with the highest distance from the group.